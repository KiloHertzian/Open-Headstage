...
impl ConvolutionEngine {
    ...
    fn convolve_path_direct(
        ...
        assert_eq!(input_signal.len(), output_buffer.len(), "Input and output buffer lengths must match.");
        assert_eq!(delay_len, ir_len.saturating_sub(1), "Delay line length mismatch for IR.");
        ...
        for i in 0..input_signal.len() {
            ...
            for k in 1..ir_len {
                if k - 1 < delay_len { // Should always be true if assert_eq passed
    ...
    pub fn process_block(
        ...
        assert_eq!(output_left.len(), block_size, "Output left channel length mismatch.");
        assert_eq!(output_right.len(), block_size, "Output right channel length mismatch.");
        ...
        Self::convolve_path_direct(input_left, &self.ir_lsl, &mut self.delay_lsl, &mut lsl_out_block);
        Self::convolve_path_direct(input_left, &self.ir_lsr, &mut self.delay_lsr, &mut lsr_out_block);
        Self::convolve_path_direct(input_right, &self.ir_rsl, &mut self.delay_rsl, &mut rsl_out_block);
        Self::convolve_path_direct(input_right, &self.ir_rsr, &mut self.delay_rsr, &mut rsr_out_block);
...
mod tests {
    ...
    fn test_default_trait() {
        ...
        assert_eq!(engine_default.ir_lsl, engine_new.ir_lsl, "Default LSL IR matches new()");
        assert_eq!(engine_default.ir_rsr, engine_new.ir_rsr, "Default RSR IR matches new()");
        assert_eq!(engine_default.delay_lsl.len(), engine_new.delay_lsl.len(), "Default LSL delay len matches new()");
        ...
        engine_default_mut.process_block(&input_l, &input_r, &mut output_l_default, &mut output_r_default);
        ...
        assert_approx_eq_slice(&output_l_default, &output_l_new, TOLERANCE, "Output from default() matches new() for left channel");
        assert_approx_eq_slice(&output_r_default, &output_r_new, TOLERANCE, "Output from default() matches new() for right channel");
    }
    ...
    fn test_set_ir_updates_ir_and_delay_line() {
        ...
        assert_eq!(engine.delay_lsl.len(), test_ir.len() - 1, "LSL delay line length after set_ir");
        assert!(engine.delay_lsl.iter().all(|&x| x == 0.0), "LSL delay line initialized to zeros");
        ...
        assert!(engine.ir_lsr.is_empty(), "LSR IR after set_ir with empty vec");
    ...
    fn test_identity_ir_single_path_passthrough() {
        ...
        assert_approx_eq_slice(&output_left, &input_signal, TOLERANCE, "Identity LSL path output_left");
        assert_approx_eq_slice(&output_right, &silent_input, TOLERANCE, "Identity LSL path output_right (should be silent)");
        ...
        assert_approx_eq_slice(&output_left2, &input_signal2, TOLERANCE, "Identity LSL path second block");
    }
    ...
    fn test_delay_ir_single_path() {
        ...
        engine.process_block(&input_signal, &silent_input, &mut output_left, &mut output_right);
        assert_approx_eq_slice(&output_left, &expected_output, TOLERANCE, "1-sample delay LSL path");
        assert_approx_eq_slice(&output_right, &silent_input, TOLERANCE, "1-sample delay LSL path (output_right silent)");
    }
    ...
    fn test_simple_filter_ir_single_path() {
        ...
        engine.process_block(&input_signal, &silent_input, &mut output_left, &mut output_right);
        assert_approx_eq_slice(&output_left, &expected_output, TOLERANCE, "Simple filter LSL path");
    }
    ...
    fn test_stereo_interaction_default_passthrough() {
        ...
        assert_approx_eq_slice(&output_l, &input_l, TOLERANCE, "Stereo default passthrough output_l");
        assert_approx_eq_slice(&output_r, &input_r, TOLERANCE, "Stereo default passthrough output_r");
    }
    ...
    fn test_stereo_interaction_full_mix() {
        ...
        let expected_sum_output: Vec<f32> = input_l.iter().zip(input_r.iter()).map(|(l, r)| l + r).collect();

        assert_approx_eq_slice(&output_l_buf, &expected_sum_output, TOLERANCE, "Stereo full mix output_l");
        assert_approx_eq_slice(&output_r_buf, &expected_sum_output, TOLERANCE, "Stereo full mix output_r");
    }
    ...
    fn test_stereo_interaction_specific_paths_with_delay() {
        ...
        assert_approx_eq_slice(&output_l, &expected_output_l, TOLERANCE, "Specific paths: output_l");
        assert_approx_eq_slice(&output_r, &expected_output_r, TOLERANCE, "Specific paths: output_r (with delay)");
    }
    ...
    fn test_process_multiple_blocks_maintains_delay_state() {
        ...
        engine.process_block(&input_block1, &silent_input_block, &mut output_block1_l, &mut output_block1_r);
        assert_approx_eq_slice(&output_block1_l, &[0.0, 0.0], TOLERANCE, "Multi-block delay: block 1 output");
        ...
        engine.process_block(&input_block2, &silent_input_block, &mut output_block2_l, &mut output_block2_r);
        assert_approx_eq_slice(&output_block2_l, &[1.0, 2.0], TOLERANCE, "Multi-block delay: block 2 output");
        ...
        engine.process_block(&input_block3, &silent_input_block, &mut output_block3_l, &mut output_block3_r);
        assert_approx_eq_slice(&output_block3_l, &[3.0, 4.0], TOLERANCE, "Multi-block delay: block 3 output (flush)");
    }
    ...
    fn test_empty_ir_results_in_silence_for_that_path() {
        ...
        engine.process_block(&input_signal, &input_signal, &mut output_left, &mut output_right);
        ...
        assert_approx_eq_slice(&output_left, &expected_output_left, TOLERANCE, "Empty LSL IR should mute left output contribution from left input");
        assert_approx_eq_slice(&output_right, &expected_output_right, TOLERANCE, "Empty LSL IR should not affect RSR path on right output");
    }
}
