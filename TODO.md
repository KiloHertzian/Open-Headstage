...
- [x] **[DEBUG] Verify Persistent Saving of Audio Backend Settings:**
    - **Description:** After multiple failed attempts to implement a manual standalone host, the immediate priority is to verify that the UI for the audio backend selector is correctly saving and loading its state. This isolates the UI and persistence logic from the complex audio backend code.
    - **Priority:** Highest.
    - **Verification Steps:**
        1. Run the standalone application (`cargo run --release`).
        2. Navigate to the "System Settings" section.
        3. Change the "Host" and "Device" dropdowns to non-default values.
...
*   **Lesson (Documentation):** Applying graphic design principles (e.g., color theory, layout, semantic shapes) to Mermaid diagrams significantly improves their clarity and maintainability. The code for the diagram should be as clean as the code for the application.