...
*   **Root Cause Analysis:** The core of the problem was a deep and persistent misunderstanding of the `nih-plug` state management and parameter system. I made several critical, incorrect assumptions:
    ...
    4.  **`set_value()` vs. `set_plain_value()`:** The correct method to programmatically change a parameter's value is `set_value()`. However, this method is not available on the `Param` objects themselves. It can only be called through a `ParamSetter` context, which is only available within the `editor()` closure. This is a critical architectural constraint.
*   **The Correct, Simple, and Robust Solution:** The simplest and most effective way to implement state persistence is to use a dedicated, serializable struct that mirrors the *values* of the parameters, and then manually copy the data to and from the `Params` object at the correct times.
    ...
    3.  **Implement `OpenHeadstagePlugin::default()`:** On startup, this function attempts to load and deserialize the `StandaloneConfig` from the JSON file. It then uses the `param.set_value()` method (which *is* available on the `Param` objects during initialization) to set the value of each parameter from the loaded config.
    4.  **Connect to the UI:** The "Save Settings" button in the UI should call the `save_standalone_config()` function.
*   **Definitive Lesson Learned:**
    ...
    3.  **Use `.value()` to Get, `.set_value()` to Set:** When interacting with parameters programmatically, always use the `.value()` method to get the current value and the `.set_value()` method to set it. The compiler errors I encountered were a direct result of trying to use incorrect, non-existent methods.
